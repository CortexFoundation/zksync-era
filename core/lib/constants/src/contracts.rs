...
/// The number that is used in the shared bridge to present ETH token. Note that it is not an
/// address of any token, but rather a representation of the ETH token, since it does not have any
/// formal address.
...
/// Default `ERA_CHAIN_ID`. All hyperchains start with this chain id and later on during their
/// registration an "initial upgrade" transaction overrides it with the correct value.
