...
pub struct IntrinsicSystemGasConstants {
    // The overhead for each L2 transaction in computation (it is assumed that it is roughly
    // independent of its structure)
    pub l2_tx_intrinsic_gas: u32,
    // The overhead for each L2 transaction in pubdata (it is assumed that it is roughly
    // independent of its structure)
    pub l2_tx_intrinsic_pubdata: u32,
    // The number of gas the refund transfer requires
    pub l2_tx_gas_for_refund_transfer: u32,
    // The overhead for each L1 transaction in computation (it is assumed that it is roughly
    // independent of its structure)
    pub l1_tx_intrinsic_gas: u32,
    // The overhead for each L1 transaction in pubdata (it is assumed that it is roughly
    // independent of its structure)
    pub l1_tx_intrinsic_pubdata: u32,
    // The minimal price that each L1 transaction should cost to cover mandatory non-intrinsic
    // parts. For instance, the user should be always able to hash at least some minimal
    // transaction. (it is assumed that it is roughly independent of its structure)
    ...
    // bytes require a new keccak round and the length of the encoding increases by 32 bytes at a
    // time)
...
/// It comes from the 64 bytes of additional two words for the bytecode length and offset in the
/// ABI-encoding of the commitment. The other "36" bytes are mostly an approximation of the amount
/// of gas it takes to properly hash it and compare with the corresponding L2->L1 message.
