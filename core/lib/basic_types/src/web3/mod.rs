...
// `AccessList`, `AccessListItem`, `TransactionReceipt`, `SignedTransaction`: from
...
pub struct FeeHistory {
    ...
    /// A vector of block base fees per gas. This includes the next block after the newest of the
    ...
    /// A vector of block gas used ratios. These are calculated as the ratio of gas used and gas
    /// limit.
    #[serde(default)] // some node implementations skip empty lists
    pub gas_used_ratio: Vec<f64>,
    /// A vector of effective priority fee per gas data points from a single block. All zeroes are
    /// returned if the block is empty. Returned only if requested.
