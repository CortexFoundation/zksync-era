...
mod tests {
    ...
    // General test.
    // It first creates wrappers of all possible size [1..16], missing only 1 byte for a full last
    // blob. Then it checks that all blobs are filled, but the last one.
    // Additional sanity check at the end ensures that the rest of the structure contains `None`s,
    // if no blobs were provided.
    #[test]
    ...
    // Additional sanity check at the end ensures that the rest of the structure contains `None`s,
    // if no blobs were provided. The only difference from the previous test is that the last
    // blob is filled.
    #[test]
