...
/// In this version of the VM the used bootloader memory bytes has increased from `24_000_000` to
/// `30_000_000`.
...
/// One of "worst case" scenarios for the number of state diffs in a batch is when 260 kb of pubdata
/// is spent on repeated writes, that are all zeroed out. In this case, the number of diffs is 260
/// kb / 5 = 52k. This means that they will have accommodate 14144000 bytes of calldata for the
/// uncompressed state diffs. Adding 260k on top leaves us with roughly 14404000 bytes needed for
/// calldata. 450125 slots are needed to accommodate this amount of data. We round up to 451000
/// slots just in case.
///
...
/// We need to limit the number of gas that can be passed into the L1->L2 transaction due to the
/// fact that unlike L2 transactions they can not be rejected by the operator and must be executed.
/// In turn, this means that if a transaction spends more than `MAX_GAS_PER_TRANSACTION`, it use up
/// all the limited resources of a batch.
///
/// It the gas limit cap on Mailbox for a priority transactions should generally be low enough to
/// never cross that boundary, since artificially limiting the gas price is bad UX. However, during
/// the transition between the pre-1.4.1 fee model and the 1.4.1 one, we need to process such
