...
pub(crate) fn derive_overhead(
    ...
    // in `O(1)` `let max_pubdata_in_tx = ceil_div_u256(gas_limit, gas_price_per_pubdata);`
...
/// All of the coefficients should be <= 1. There are here to provide a certain "discount" for
/// normal transactions at the risk of malicious transactions that may close the block prematurely.
/// IMPORTANT: to perform correct computations, `MAX_TX_ERGS_LIMIT /
/// coefficients.ergs_limit_overhead_coeficient` MUST result in an integer number
...
impl OverheadCoefficients {
    ...
    fn new_l2() -> Self {
        OverheadCoefficients::new_checked(
            1.0, 1.0,
            // For L2 transactions we allow a certain default discount with regard to the number of
            // ergs. Multi-instance circuits can in theory be spawned infinite times,
            // while projected future limitations on gas per pubdata allow for roughly
            // 800k gas per L1 batch, so the rough trust "discount" on the proof's part
...
pub(crate) fn get_amortized_overhead(
    ...
    //```
    // TODO (EVM-67): possibly include the overhead for pubdata. The formula below has not been
    // properly maintained, since the pubdat is not published. If decided to use the pubdata
    ...
    // MAX_PUBDATA_PER_BLOCK). Throwing off the `ceil`, while may provide marginally lower
    ...
    // `OE = floor(OB * TL + MAX_TX_ERGS_LIMIT / (MAX_TX_ERGS_LIMIT/K + OB))`, with possible -1 if
    // the division is without remainder
