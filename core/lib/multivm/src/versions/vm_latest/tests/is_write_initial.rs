...
fn test_is_write_initial_behaviour() {
    ...
    // The main idea is to check that `is_write_initial` storage uses the correct cache for
    // initial_writes and doesn't messed up it with the repeated writes during the one batch
    // execution.
