...
/// The amount of ergs to be reserved at the end of the batch to ensure that it has enough ergs to
/// verify compression, etc.
...
/// In the version `1.5.0` the maximal number of circuits per batch has been increased from `24100`
/// to `34100`.
...
/// In this version of the VM the used bootloader memory bytes has increased from `30_000_000` to
/// `59_000_000`, and then to `63_800_000` in a subsequent upgrade.
...
/// We want `MAX_GAS_PER_PUBDATA_BYTE` multiplied by the u32::MAX (i.e. the maximal possible value
/// of the pubdata counter) to be a safe integer with a good enough margin.
/// Note, that in the version `1.5.0` we've only increased the max gas per pubdata byte to 50k for
/// backward compatibility with the old SDK, where the default signed value is 50k.
...
/// One of "worst case" scenarios for the number of state diffs in a batch is when 780kb of pubdata
/// is spent on repeated writes, that are all zeroed out. In this case, the number of diffs is
/// `780kb / 5 = 156k`. This means that they will have accommodate 42432000 bytes of calldata for
/// the uncompressed state diffs. Adding 780kb on top leaves us with roughly 43212000 bytes needed
/// for calldata. 1350375 slots are needed to accommodate this amount of data. We round up to
/// 1360000 slots just in case.
///
/// In theory though much more calldata could be used (if for instance 1 byte is used for enum
/// index). It is the responsibility of the operator to ensure that it can form the correct calldata
/// for the L1Messenger.
...
/// Method that provides the start position of the vm hook in the memory for the latest version of
/// v1.5.0. This method is used only in `test_infra` in the bootloader tests and that's why it
/// should be exposed.
...
/// The maximal number of gas that is supposed to be spent in a batch. This value is displayed in
/// the system context as well as the API for each batch.
/// Using any number that fits into `i64` is fine with regard to any popular eth node
/// implementation, but we also desire to use values that fit into safe JS numbers just in case for
/// compatibility.
...
/// The maximal gas limit that gets passed as compute for an L2 transaction. This is also the
/// maximal limit allowed for L1->L2 transactions.
///
/// It is equal to the `MAX_GAS_PER_TRANSACTION` in the bootloader.
/// For L2 transaction we need to limit the amount of gas that can be spent on compute while the
/// rest can only be spent on pubdata. For L1->L2 transactions We need to limit the number of gas
/// that can be passed into the L1->L2 transaction due to the fact that unlike L2 transactions they
/// can not be rejected by the operator and must be executed. In turn, this means that if a
/// transaction spends more than `MAX_GAS_PER_TRANSACTION`, it use up all the limited resources of a
/// batch.
...
/// It is roughly equal to `80kk/MAX_TRANSACTIONS_PER_BATCH`, i.e. how many gas would an L1->L2
/// transaction need to pay to compensate for the batch being closed.
/// While the derived formula is used for the worst case for L1->L2 transaction, it suits L2
/// transactions as well and serves to compensate the operator for the fact that they need to
/// process the transaction. In case batches start getting often sealed due to the slot limit being
/// reached, the L2 fair gas price will be increased.
...
/// It is roughly equal to `80kk/BOOTLOADER_MEMORY_FOR_TXS`, i.e. how many gas would an L1->L2
/// transaction need to pay to compensate for the batch being closed.
/// While the derived formula is used for the worst case for L1->L2 transaction, it suits L2
/// transactions as well and serves to compensate the operator for the fact that they need to fill
/// up the bootloader memory. In case batches start getting often sealed due to the memory limit
/// being reached, the L2 fair gas price will be increased.
