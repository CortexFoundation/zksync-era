...
// Special for `zk_evm_1_3_1`: it re-used the same sorting function from `zkevm_test_harness` as the
// `v1.3.3` used. To continue calling this functions, we need to add the conversion for `Timestamp`
// and `LogQuery`.
