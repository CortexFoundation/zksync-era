...
/// This is the gas limit that was used inside blocks before we started saving block gas limit into
/// the database.
...
pub(crate) struct StorageL1BatchHeader {
    ...
    // `system_logs` are introduced as part of boojum and will be absent in all batches generated
    // prior to boojum. System logs are logs generated by the VM execution, rather than
    // directly from user transactions, that facilitate sending information required for
    // committing a batch to l1. In a given batch there will be exactly 7 (or 8 in the event of
    // a protocol upgrade) system logs.
...
pub(crate) struct StorageL2BlockHeader {
    ...
    // `min(virtual_blocks`, `miniblock_number - virtual_block_number`), i.e. making sure that
    // virtual blocks never go beyond the miniblock they are based on.
    pub virtual_blocks: i64,

    /// The formal value of the gas limit for the miniblock.
    /// This value should bound the maximal amount of gas that can be spent by transactions in the
    /// miniblock.
