...
impl EthSenderDal<'_, '_> {
    ...
    /// This method inserts a fake transaction into the database that would make the corresponding
    /// L1 batch to be considered committed/proven/executed.
    ///
    /// The designed use case is the External Node usage, where we don't really care about the
    /// actual transactions apart from the hash and the fact that tx was sent.
    ///
    /// ## Warning
    ///
    /// After this method is used anywhere in the codebase, it is considered a bug to try to
    /// directly query `eth_txs_history` or `eth_txs` tables.
