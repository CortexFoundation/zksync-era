...
/// Note, that the `min_allowance` will be approved to paymaster contract during estimation and so
/// for instance "low" values like zero could lead to underestimation of the transaction (because
/// the cost per write depends on the size of the value).
///
/// This is a chicken-and-egg problem, because we need to know the cost of the transaction to
/// estimate it, so in order to provide the most correct estimate possible it is recommended to set
/// as realistic `min_allowance` as possible.
