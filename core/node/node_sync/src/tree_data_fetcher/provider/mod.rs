...
pub(crate) enum MissingData {
    ...
    Batch,
    /// The provider lacks a root hash for a requested L1 batch; the batch itself is present on the
    /// provider.
...
pub(crate) trait TreeDataProvider: fmt::Debug + Send + Sync + 'static {
    ...
    ///
    /// It is guaranteed that this method will be called with monotonically increasing `number`s
    /// (although not necessarily sequential ones).
...
/// Provider of tree data loading it from L1 `BlockCommit` events emitted by the diamond proxy
/// contract. Should be used together with an L2 provider because L1 data can be missing for latest
/// batches, and the provider implementation uses assumptions that can break in some corner cases.
///
/// # Implementation details
///
/// To limit the range of L1 blocks for `eth_getLogs` calls, the provider assumes that an L1 block
/// with a `BlockCommit` event for a certain L1 batch is relatively close to L1 batch sealing. Thus,
/// the provider finds an approximate L1 block number for the event using binary search, or uses an
/// L1 block number of the `BlockCommit` event for the previously queried L1 batch (provided it's
/// not too far behind the seal timestamp of the batch).
#[derive(Debug)]
...
impl L1DataProvider {
    ...
    /// `L1_BLOCK_ACCURACY`, but not large enough to trigger request limiting on the L1 RPC
    /// provider.
    ...
    async fn guess_l1_commit_block_number(
        ...
        // At this point, we have `earliest_timestamp <= l1_batch_seal_timestamp <=
        // latest_timestamp`. Binary-search the range until we're sort of accurate.
...
impl TreeDataProvider for L1DataProvider {
    async fn batch_details(
        ...
        match logs.as_slice() {
            ...
            _ => {
                tracing::warn!(
                    "Non-unique `BlockCommit` event for L1 batch #{number} queried using {filter:?}: {logs:?}"
                );
        ...
        if let Some(l1) = &mut self.l1 {
            match l1.batch_details(number).await {
                ...
                Ok(Err(missing_data)) => {
                    ...
                    // No sense of calling the L1 provider in the future; the L2 provider will very
                    // likely get information about batches significantly
                    // faster.
                    self.l1 = None;
                }
            }
