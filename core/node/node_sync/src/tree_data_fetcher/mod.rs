...
impl TreeDataFetcher {
    ...
    /// Attempts to fetch root hashes from L1 (namely, `BlockCommit` events emitted by the diamond
    /// proxy) if possible. The main node will still be used as a fallback in case communicating
    /// with L1 fails, or for newer batches, which may not be committed on L1.
    pub fn with_l1_data(
        mut self,
